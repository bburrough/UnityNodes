...
{
    /*
        Returns the orhtogonal distance between the point and the line.
    */
    private static float DistancePointLineSegment(Vector2 lineStart, Vector2 lineEnd, Vector2 point)
    {
        // 2D arithmetic not implemented
        throw new System.NotImplementedException();
    }


    //private static bool AreParallel(Vector2 position_a, Vector2 direction_a, Vector2 position_b, Vector2 direction_b)
    //{
    //    throw new System.NotImplementedException();        
    //}
    ...
    /*
        Compute the intersection point between the line segments described
        by p1->p2 and p3->p4. Returns true if the lines intersect, otherwise
        returns false.
    ...
    */
    ...
    public static void MostDistantPoints(List<Vector2> points, out Vector2 first_point, out Vector2 second_point)
    {
        first_point = Vector2.zero;
        second_point = Vector2.zero;

        if (points == null)
            throw new System.ArgumentException("points is null");
        if (points.Count < 2)
            throw new System.ArgumentException("Insufficient number of points.");

        float max_sqr_magnitude = float.NegativeInfinity;

        for(int i = 0; i < points.Count - 1; i++)
        {
            for(int j = i + 1; j < points.Count; j++)
            {
                float temp_sqr_magnitude = Vector2.SqrMagnitude(points[i] - points[j]);
                if(temp_sqr_magnitude > max_sqr_magnitude)
                {
                    first_point = points[i];
                    second_point = points[j];
                    max_sqr_magnitude = temp_sqr_magnitude;
                }
            }
        }
    }
    ...
    public static float SignedAngle(Vector2 left_edge, Vector2 right_edge)
    {
        float cross_factor = left_edge.x * right_edge.y - left_edge.y * right_edge.x;
        float dot_factor = left_edge.x * right_edge.x + left_edge.y * right_edge.y;

        float signed_angle_between = Mathf.Atan2(cross_factor, dot_factor) * Mathf.Rad2Deg;

        return signed_angle_between;
    }
}
...
public class Math3d : MonoBehaviour {
    ...
    public static bool LineSegmentPlaneIntersection(out Vector3 intersection, Vector3 point1, Vector3 point2, Vector3 planeNormal, Vector3 planePosition)
    {
        Vector3 secondaryIntersection = Vector3.zero;
        if (Math3d.LinePlaneIntersection(out secondaryIntersection, point1, (point2 - point1).normalized, planeNormal, planePosition))
        {
            if (0 == Math3d.PointOnWhichSideOfLineSegment(point1, point2, secondaryIntersection))
            {
                intersection = secondaryIntersection;
                return true;
            }
        }
        intersection = Vector3.zero;
    ...
    /*
        Returns the orthogonal distance between the point and the line segment described by lineStart <-> lineEnd.
    */
    public static float DistancePointLineSegment(Vector3 lineStart, Vector3 lineEnd, Vector3 point)
    {
        return Vector3.Magnitude(Math3d.ProjectPointOnLineSegment(lineStart, lineEnd, point) - point);
    }




    public static float DistancePointLine(Vector3 lineStart, Vector3 lineVec, Vector3 point)
    {
        return Vector3.Magnitude(Math3d.ProjectPointOnLine(lineStart, lineVec, point) - point);
    }
